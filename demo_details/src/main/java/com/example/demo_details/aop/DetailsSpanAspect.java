package com.example.demo_details.aop;
...
@Aspect
@Component
public class DetailsSpanAspect {
    ...
    Logger logger = LoggerFactory.getLogger(DetailsSpanAspect.class);
     ...
     *              com.example.demo_details.controller.DetailsController.*(..) 패턴으로 Around AOP를 적용합니다.
     */    
    @Around("execution(* com.example.demo_details.controller.DetailsController.*(..))")
    ...
    public HttpHeaders makeHttpHeaders(HttpHeaders httpHeaders) {
        ...
        for (String header : headers_to_propagate) {
            if (httpHeaders.containsKey(header)) {
                logger.info("==========================  [ makeHttpHeaders : AOP ] KEY : " + header + ", VALUE : "
     ...
     *              com.example.demo_details.controller.DetailsController.get*(..) 패턴으로 Around AOP를 적용합니다.
     */    
    @Around("execution(* com.example.demo_details.controller.DetailsController.get*(..))")
     ...
     * @return      Object
     * @description ProceedingJoinPoint에서 Args로 넘어온 HttpHeaders를 가져와서 makeHttpHeaders Method를 사용하여 Http Header 정보를 Exchange한다.
     *              ccom.example.demo_details.controller.DetailsController.post*(..) 패턴으로 Around AOP를 적용합니다.
     */    
    @Around("execution(* com.example.demo_details.controller.DetailsController.post*(..))")
    public Object headerExchangePost(ProceedingJoinPoint pjp) throws Throwable {
        logger.info("[                             headerExchangePost Start                                ]");
        HttpHeaders headers = null;
        DetailsDTO.Request request = null;
        ...
        for(Object obj : pjp.getArgs()) {
            ...
            if(obj instanceof DetailsDTO.Request) {
                request = (DetailsDTO.Request) obj;
            }
        }
